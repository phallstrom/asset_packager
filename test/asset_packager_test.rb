...
class AssetPackagerTest < Test::Unit::TestCase
  ...
  def test_find_by_type
    ...
    assert_equal 5, js_asset_packages.length
  ...
  def test_delete_and_build
    ...
    assert_equal 5, js_package_names.length
    assert_equal 4, css_package_names.length
    assert_equal 1, css_subdir_package_names.length
    assert js_package_names[0].match(/\Abase_packaged.js\z/)
    assert js_package_names[1].match(/\Acompress_test_packaged.js\z/)
    assert js_package_names[2].match(/\Adot_test_packaged.js\z/)
    assert js_package_names[3].match(/\Arootlevel_packaged.js\z/)
    assert js_package_names[4].match(/\Asecondary_packaged.js\z/)
    assert css_package_names[0].match(/\Abase_packaged.css\z/)
    assert css_package_names[1].match(/\Adot_test_packaged.css\z/)
    assert css_package_names[2].match(/\Arootlevel_packaged.css\z/)
    assert css_package_names[3].match(/\Asecondary_packaged.css\z/)
